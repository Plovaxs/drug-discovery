...
| D.5b | Follow-up: third proxy (heteroatom fraction), requested during thesis-writing review | Also ruled out: corr=0.042 [-0.008,0.091], CI includes 0 (n=144, same scale as D.5). Three candidate proxies now tested (size, aromaticity, heteroatom fraction); mechanism remains open | 2026-09-25 | `guidance/DIAG_SYNTH_DIRECTION_FINDING.md`'s follow-up section; `guidance/diag_synth_direction_followup.py`; `guidance/diag_synth_direction_followup_results.json` |

**Track D closed.** Overall conclusion: divergence persists after two independent fix attempts (bond-aware features, leakage-safe retrain) and remains mechanistically unexplained after three tested proxies (size/heaviness, aromaticity, heteroatom fraction) across two diagnostic rounds (D.5, D.5b) -- see `guidance/DIAG_SYNTH_DIRECTION_FINDING.md`'s closing table for the full D.2-D.5 summary.
