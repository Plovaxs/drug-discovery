...
## Addendum 2 (2026-09-25): research-frontier scan supplied by the user — verification status

**Gao, B.; Tan, H.; Huang, Y.; Ren, M.; Huang, X.; Ma, W.-Y.; Zhang, Y.-Q.; Lan, Y.** *From Theory to Therapy: Reframing SBDD Model Evaluation via Practical Metrics.* arXiv:2406.08980 (2024) [**preprint**; the ICLR 2025 version, "Reframing Structure-Based Drug Design Model Evaluation via Metrics Correlated to Practical Needs", was **not** read]. Read from the arXiv PDF text: Vina scores inflate with atom count; diffusion SBDD models such as TargetDiff "may be overfitting to Vina scores"; CrossDocked is cross-docked/synthesised data, not crystal structures. Treat as a required citation for the size/Vina-gaming side of the thesis, but **do not quote** the "hacking of Vina scores" sentence or the checkpoint-selection-leakage statement until confirmed in the ICLR version. Also note its evidence is about evaluation metrics, not guidance gradients.

Supplied but **not verified by me** (future-work / limitations pointers only): Apo2Mol (arXiv:2511.14559, dynamic pockets), DynamicFlow (2025), *Brief. Bioinform.* 2025 flexible-docking review (bbaf454), "Synthesizability via Reward Engineering" (ChemRxiv 2025 / *Chem. Sci.* 2025; full text blocked), PMC11897179 (reward hacking in multi-objective design), UQ papers (*Nat. Commun.* 2025, s41467-025-58503-0; *JCIM* 2025, 5c02381), PMC12401186, *Molecules* 30 (16) 3361 (GNINA vs Vina), PMC12613558 (evaluation choices in generative drug discovery). Rigid-pocket assumption, synthesizability reward hacking and uncertainty quantification for guidance signals are reasonable one-sentence limitations/future-work items; none is in scope. DiffInt's training loss remains unconfirmed (the ChemRxiv page returned HTTP 403 for me too).

**Update to Addendum 2 (2026-09-25):** the ICLR 2025 camera-ready of Gao et al. (*Reframing Structure-Based Drug Design Model Evaluation via Metrics Correlated to Practical Needs*) was read from its PDF text: the "hacking of Vina scores" sentence (Sec. 4.5.1) and the "test set as a validation set to select checkpoints… risks data leakage" sentence are **confirmed verbatim** there and absent from arXiv:2406.08980. Cite the ICLR version for these; see `TRACK_E_DESIGN.md` A10 for exact wording and scope.
