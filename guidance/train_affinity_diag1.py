"""Diagnostic 1 (training-size learning curve): identical training/eval
loop to guidance/train_affinity_model.py, but builds its train/val split
from a FRACTION of the 507-target pool (guidance/target_subsample_splits.py)
instead of build_labeled_splits' flat example-count subsample. The held-out
27-pocket test set is unchanged in both scripts.

Kept as a separate script (not a flag on train_affinity_model.py) because
the split-construction call itself differs, not just a config value --
everything else (model, optimizer, train/validate loop, checkpointing) is
identical to train_affinity_model.py by design, so the two stay directly
comparable.
"""
import argparse
import os
...
from guidance.target_subsample_splits import build_target_fraction_splits, verify_no_test_overlap
...
def main():
    ...
    parser.add_argument('--target_fraction', type=float, required=True)
    parser.add_argument('--device', type=str, default='cuda')
    parser.add_argument('--logdir', type=str, default='./logs_affinity_diag1')
    ...
    logger = misc.get_logger('train_affinity_diag1', log_dir)
    ...
    logger.info(f'Building target-fraction={args.target_fraction} CrossDocked2020 affinity splits...')
    base, splits, pk_by_idx, selected_targets = build_target_fraction_splits(
        args.target_fraction, root=config.dataset.root, split_path=config.dataset.split,
    ...
    logger.info(f'Target fraction {args.target_fraction}: n_targets={len(selected_targets)}  '
               f'Train: {len(splits["train"])}  Val: {len(splits["val"])}  Test: {len(splits["test"])}  '
               f'-- no train/test target overlap confirmed')
